...
def concat_logs_path(args):

    if args.decentralized:
        return f"{args.dataset}_decentralized"
    # Transform experiment's arguments into a string. Can be used to concat log dirs
    args_string = ""
    args_to_show = ["dataset","algorithm"]
...
def parse_args(args_list=None):
    ...
    parser.add_argument('algorithm',
                        ...
                        'possible are `FedAvg`,`FedEM`,`FedEdg`,`FedProx`;',
                        type=str)
    parser.add_argument(
        '--decentralized',
    ...
    parser.add_argument('--input_dimension',
                        ...
                        type=int,
                        default=None)
    parser.add_argument('--output_dimension',
                        help='the dimension of output space; only used for synthetic dataset',
                        type=int,
                        default=None)
    parser.add_argument('--n_learners',
                        help='number of learners_ensemble to be used with `FedEM`;'
                             'ignored if algorithm is not `FedEM`;'
                             'default is 3',
                        type=int,
                        default=3)
    ...
    parser.add_argument(
        "--logs_dir",
        help='directory to write logs; if not passed, it is set None',
